...
class TreeData(Dataset):
    ...
    def __init__(
        ...
        if data_augmentation:
            ...
            print(
                "Rotating the trees systematically in {} other positions: {}".format(
                    self.num_rot,
                    ", ".join(
                        map(
                            lambda x: "{:.1f}".format(x) + "°", rot_angles * 180 / np.pi
                        )
                    ),
                )
            )
        ...
        if self.transform is not None:
            ...
            print(
                "Transforming point clouds to {} 3DmFV".format(
    ...
    def __getitem__(self, idx):
        sample = {
            "points": tensor(self.trees[idx]).float(),
    ...
    def __str__(self):
        ...
        return "TreeData(numtrees={}, transform={}{}, data_augmentation={}{})".format(
            self.numtrees,
            self.transform is not None,
            ", grid={}".format(self.transform) if self.transform is not None else "",
            self.data_augmentation,
            ", rotations={}".format(self.num_rot) if self.transform else "",
        )
...
class CMDisplay:
    def __init__(
        ...
        confusion_matrix,
        *,
        display_labels=["coniferous", "decidious", "snag", "dead tree"]
    ):
...
class Inception(torch.nn.Module):
    ...
    def __init__(
        ...
        self.cnn0 = Seq(
            Conv3d(in_channels=in_channels, out_channels=n_filters, kernel_size=1),
            BatchNorm3d(n_filters),
        )
        self.avgpool = Seq(
            ConstantPad3d(self.pad_c1, 0), AvgPool3d(kernel_size=kernel_c1, stride=1)
        )
        self.cnn1 = Seq(
...
def get_3D_grid(subdivisions=[8, 8, 8], std=None, astensor=False):
    ...
    D = divisions.shape[0]
    means = (
        np.r_["-1", np.meshgrid(*([np.linspace(-1, 1, d) for d in divisions]))]
        .reshape(D, -1)
        .T
    )
    std = std * np.ones_like(means)
    w = (1.0 / n_gaussians) * np.ones((n_gaussians, 1))
    if astensor:
...
def get_3DmFV(tree, mu, sigma, w=None):
    ...
    prob_tree = tree - mu
    prob_tree = np.exp(np.sum(-0.5 * prob_tree ** 2 * sigma, axis=-1))  # [N, K]
    ...
    d_alpha = np.stack((d_alpha.max(axis=0), d_alpha.sum(axis=0)), axis=1)  # [K, 2]

    d_mu = (
        prob_tree[:, :, np.newaxis]
        * ((tree - mu) / sigma)
        / (np.sqrt(w[:, np.newaxis]) * N)
    )  # [N, K, D]
    d_mu = np.concatenate(
        (d_mu.max(axis=0), d_mu.min(axis=0), d_mu.sum(axis=0)), axis=1
    )  # [K, D*3]

    d_sigma = (
        prob_tree[:, :, np.newaxis]
        * (((tree - mu) / sigma) ** 2 - 1)
        / (np.sqrt(w[:, np.newaxis] * 2) * N)
    )  # [N, K, D]
    d_sigma = np.concatenate(
        (d_sigma.max(axis=0), d_sigma.min(axis=0), d_sigma.sum(axis=0)), axis=1
    )  # [K, D*3]
